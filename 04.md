...
10. Let's repeat the process for the user aliceanderson
    ...
    `[root@controller ~(keystone_chestercopperpot)#` `nano keystonerc_aliceanderson`
	
11. Edit **keystonerc_aliceanderson** so it looks like the following (or copy and paste is fine):
...
13. Finally, lets repeat the process again for user bobbarker
    ...
    `[root@controller ~(keystone_chestercopperpot)#` `nano keystonerc_bobbarker`
	
14. Edit **keystonerc_bobbarker** so it looks like the following (copy and paste is fine):
...
5. This file will download locally. Open it in your favorite text editor. The file you open will look similar to the one shown below.
